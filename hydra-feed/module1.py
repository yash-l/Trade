...
class FeedConfig:
    ...
    MAX_TPS: int = 8000     # SRE PATCH: Safer throughput limit for stable pipelines
    BATCH_LIMIT: int = 200  # SRE PATCH: Prevents Redis pipeline stalls
...
class TickSanitizer:
    ...
    def sanitize(self, raw_tick: dict) -> dict | None:
        try:
            ...
            now = time.time()
            # SRE PATCH: Relaxed Time Sanity Check (+2s forward, -10s backward)
            if ts > now + 2.0 or ts < now - 10.0: return None 
...
class MarketFeedGuardian:
    ...
    async def _route_batch_to_redis(self, clean_ticks: list):
        ...
        try:
            # SRE PATCH: Pipeline Chunking
            for i in range(0, len(clean_ticks), self.config.BATCH_LIMIT):
                chunk = clean_ticks[i:i + self.config.BATCH_LIMIT]
        ...
        while self._running or force:
            if self.local_buffer and self.redis:
                try:
                    pipe = self.redis.pipeline(transaction=False)
                    count = 0
                    # SRE PATCH: Apply batch limit to flushing as well
                    while self.local_buffer and count < self.config.BATCH_LIMIT:
